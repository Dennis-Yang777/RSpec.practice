...
# actor = Actor.new('Brad')
# movie = Movie.new(actor)
# movie.start_shooting

RSpec.describe Movie do
	let(:stuntman) { double("Mr. Danger", ready?: true, act: 'string', fall_off_ladder: 'Do it', light_on_fire: false) }
	...
	describe '#start_shooting method' do
		it 'expects an actor to do 3 actions' do
			...
			expect(stuntman).to receive(:act).exactly(2).at_least(2).times
